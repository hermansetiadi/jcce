...
def figure1b_pilot():
    ...
    if not ov_dir.exists():
        ...
        return
    # Accept both the old "overlay_*" and current "pilot_*" naming, and do NOT
    # hardcode tile names: the spatially-blocked split changed which tiles are in
    # the holdout, so the previously named tiles may not exist at all.
    files = sorted(ov_dir.glob("*.png"))[:2]
    if not files:
    ...
    for ax, f in zip(axes, files):
        ax.imshow(mpimg.imread(f))
        ax.axis("off")
        name = f.stem.replace("overlay_", "").replace("pilot_", "")
        ax.set_title(f"Tangerang pilot · {name}", fontsize=9, loc="left")
    # The caption states what the panels SHOW, not a result. The previous text
    # asserted that Setup A floods false positives and Setup B suppresses them --
    # a claim the corrected pipeline does not support, and one that must never be
    # baked into a caption independent of the data being plotted.
    fig.suptitle("Figure 1b.  Single-region pilot (image · ground truth · Setup A · "
                 "Setup B). No-data borders are excluded from all metrics and are "
                 "never assigned a prediction.", y=1.0, fontsize=9)
    fig.tight_layout(rect=[0, 0, 1, 0.99])
...
def figure3_cross():
    ...
    for ax, subset, title in [
        ...
        # A direction has no low-coverage block when the TARGET region contributes
        # no tiles at that coverage band -- after valid-area normalisation
        # Tangerang holds only 8 such tiles region-wide, and they are spent on the
        # training/negative pool. Report that honestly instead of crashing.
        have = [d for d in order
                if d["setups"]["setup_a"].get(subset)
                and d["setups"]["setup_b"].get(subset)]
        if not have:
            ax.text(0.5, 0.5, "no low-coverage tiles\nin either target region",
                    ha="center", va="center", transform=ax.transAxes, fontsize=9,
                    color="#666666")
            ax.set_xticks([]); ax.set_yticks([]); ax.set_title(title)
            continue
        missing = [label_for(d) for d in order if d not in have]
        xs = np.arange(len(have)); labs = [label_for(d) for d in have]
        Am = [d["setups"]["setup_a"][subset]["false_positive_rate"]["mean"] for d in have]
        As = [d["setups"]["setup_a"][subset]["false_positive_rate"]["std"] for d in have]
        Bm = [d["setups"]["setup_b"][subset]["false_positive_rate"]["mean"] for d in have]
        Bs = [d["setups"]["setup_b"][subset]["false_positive_rate"]["std"] for d in have]
        x, labels_here = xs, labs
        ...
        if missing:
            ax.set_xlabel("omitted (no low-coverage tiles): "
                          + "; ".join(m.replace("\n", " ") for m in missing), fontsize=7)
    ...
    ax1.legend(frameon=False, fontsize=9)
    # Neutral caption: the per-bar "helps/hurts" annotations are computed from the
    # data, so the title must not assert a direction the data may not show.
    fig.suptitle("Figure 3.  Cross-region transfer: target-region false-positive rate "
                 "by setup and direction (3 seeds, mean ± std). No-data pixels excluded.",
                 y=1.02, fontsize=9.5)
    fig.tight_layout()
